...
namespace LogiPharm.Presentacion
{
    public partial class FrmSecuencias : Form
    {
        public FrmSecuencias()
        {
            ...
            NBitacora.Registrar(SesionActual.IdUsuario, SesionActual.NombreUsuario, "Configuración", "VISUALIZAR", "secuencias", null, "Abrir Secuencias", null, Environment.MachineName, "UI");
        }
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dgvSecuencias.Rows)
                {
                    ...
                    NBitacora.Registrar(SesionActual.IdUsuario, SesionActual.NombreUsuario, "Configuración", "EDITAR", "secuencias", null, $"Guardar secuencia '{nombre}'", null, Environment.MachineName, "UI");
                }
            ...
            if (dgvSecuencias.CurrentRow != null && !dgvSecuencias.CurrentRow.IsNewRow)
            {
                ...
                if (r == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        // Auditoría: ELIMINAR
                        NBitacora.Registrar(SesionActual.IdUsuario, SesionActual.NombreUsuario, "Configuración", "ELIMINAR", "secuencias", null, $"Eliminar secuencia '{nombre}'", null, Environment.MachineName, "UI");
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
